...
var EasyAccess = function(provider, original_options) {
  ...
  this.interactive  = options.interactive || (Boolean(process.stdin.isTTY) && Boolean(process.stdout.isTTY));
...
EasyAccess.prototype.get_access_token = function(callback) {
  ...
  self.load(function(file_data) {
    if (self.client_grant) {
      self.enter_credentials(function(err) {
        if (err) {
          callback();
        } else {
      ...
      self.enter_credentials(function(err) {
        if (err) {
          ...
          self.request_manual_authorization(callback);
        }
      });
    }
  });
...
EasyAccess.prototype.enter_credentials = function(callback) {
  var self = this;
  if (!self.interactive) {
    ...
    return callback('not_interactive');
  }

  if (!self.client_id) {
    console.error('Authenticating manually against ' + self.host);
    console.error('If you don\'t have a client_id and client_secret, please visit the site and get one');
    console.error('Verify that your callback url points to \'http://localhost:' + self.port + '/\'');
  }
...
EasyAccess.prototype.request_manual_authorization = function(callback) {
  var self = this;
...
EasyAccess.prototype.request_access_token = function(form_data, callback) {
  ...
  request.post(token_url, {
    ...
    if (error) {
      ...
      token_data.refresh_token = token_data.refresh_token || form_data.refresh_token;
      delete token_data.expires_in;

      // if we don't get scope back in the response, assume we got the scopes we asked for
      if (typeof token_data.scope === 'undefined') {
